...
static void BAS_APP_UpdateLevel(void);
/* USER CODE END PFP */
  ...
  switch(p_Notification->EvtOpcode)
  {
    ...
    case BAS_BAL_READ_EVT:
      /* USER CODE BEGIN Service3Char1_READ_EVT */
      {
        ...
        msg_conf.Length = sizeof(BAS_APP_Context.Level);
        msg_conf.p_Payload = a_BAS_UpdateCharData;
        BAS_UpdateValue(BAS_BAL, &msg_conf);
      }
...
void BAS_APP_Init(void)
{
  ...
  APP_DBG_MSG("BAS_APP_Init\n\r");

  UTIL_SEQ_RegTask( 1<< CFG_TASK_BAS_LEVEL_REQ_ID, UTIL_SEQ_RFU, BAS_APP_UpdateLevel );
